...
class Main extends PluginBase
{
    ...
    public function onEnable()
    {
        ...
        if ($this->hasBossBar()) {
            $this->getBossBar()->setTitle("§b» §l§eSKYBLOCK§r §b«");
    ...
    public function executeAddQuestsSystem(Player $player, $data = null)
    {
        ...
        $player->sendMessage("  §6§lNO OBJECTIVE\n  §r§f Finish Quest");
        $this->playerDataPath[$name]['objectives']['objectiveData'] = [];
        if ($this->hasBossBar()) {
            $this->bossBar->setTitleFor([$player], "§b» §l§eSKYBLOCK§r §b«");
        }
    }
    ...
    public function addPlayerReward(Player $player, array $cmd, array $data)
    {
        ...
        if ($island !== null) {
            ...
            $rewardSummary = "§3--------------------------------\n\n§bTotal Earned Experience: ".$total."\n\n§3--------------------------------";
    ...
    public function addScore(Player $player, string $title, array $lines)
    {
        ...
        $player->sendPopup(str_replace(["{name}", "{health}", "{max_health}", "{defense}", "{mana}", "&"], [$player->getName(), round($player->getHealth(), 2), $player->getMaxHealth(), round($this->getArmorPoints($player)), $this->getPlayerMana($player), "§"], $this->config['hud']['format']));
    }
    ...
    public function getFormatIslandTop(Player $player, string $type): string
    {
        ...
        $text .= "\n§a============§f^§a============\n   \n§eSneaking and hit to move previous\n§aHit to move next";
        return $text;
    }
    ...
    {
        $this->getServer()->getCommandMap()->register("NichiSkyBlock", new SkyBlockCommand("skyblock", $this));
    }
    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args): bool
    {
        switch ($command->getName()) {
                ...
                if ($sender instanceof Player) {
                    if (count($args) < 1) {
                        ...
                        if ($player !== null) {
                            ...
                            if ($island !== null) {
                                if ($island->isLocked()) {
                                    $this->sendMessageNonStatic($sender, "This island is locked, you cannot join it!");
                                } else {
                                    ...
                                    $this->sendMessageNonStatic($sender, "You joined the island successfully");
                                }
                            } else {
                                $this->sendMessageNonStatic($sender, "At least SkyBlock island member must be active if you want see the island!");
                            }
                        } else {
            ...
            case "addquest":
                ...
                (new PublicMenu())->addQuests($sender, "Enter New Name");
                break;
        }
        return true;
    }
